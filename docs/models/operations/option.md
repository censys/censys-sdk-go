...
```go
censyssdkgo.WithServerURL("https://api.example.com")
```
...
```go
censyssdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
censyssdkgo.WithClient(httpClient)
```
...
```go
censyssdkgo.WithSecuritySource(/* ... */)
```
...
```go
censyssdkgo.WithOrganizationID(/* ... */)
```
...
```go
censyssdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
censyssdkgo.WithTimeout(30 * time.Second)
```
