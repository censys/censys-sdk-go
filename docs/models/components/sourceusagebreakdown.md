# SourceUsageBreakdown


## Fields

| Field                                                      | Type                                                       | Required                                                   | Description                                                |
| ---------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------- |
| `API`                                                      | *int64*                                                    | :heavy_check_mark:                                         | The amount of credits consumed through the Platform API.   |
| `AutoReplenishment`                                        | **int64*                                                   | :heavy_minus_sign:                                         | The amount of credits consumed through auto-replenishment. |
| `Other`                                                    | **int64*                                                   | :heavy_minus_sign:                                         | The amount of credits consumed through other operations.   |
| `UI`                                                       | *int64*                                                    | :heavy_check_mark:                                         | The amount of credits consumed through the Platform UI.    |